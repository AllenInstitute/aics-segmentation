...
from aicssegmentation.core.output_utils import save_segmentation, CETN2_output, generate_segmentation_contour
from aicsimageprocessing import resize


def Workflow_cetn2(struct_img,rescale_ratio, output_type, output_path, fn, output_func=None):
    ##########################################################################
    # PARAMETERS:
    ...
    # rescale if needed
    if rescale_ratio>0:
        struct_img_for_seg = resize(struct_img_for_seg, [1, rescale_ratio, rescale_ratio], method="cubic")
        struct_img_for_seg = (struct_img_for_seg - struct_img_for_seg.min() + 1e-8)/(struct_img_for_seg.max() - struct_img_for_seg.min() + 1e-8)
        struct_img_for_peak = resize(struct_img_for_peak, [1, rescale_ratio, rescale_ratio], method="cubic")
        struct_img_for_peak = (struct_img_for_peak - struct_img_for_peak.min() + 1e-8)/(struct_img_for_peak.max() - struct_img_for_peak.min() + 1e-8)
        gaussian_smoothing_truncate_range = gaussian_smoothing_truncate_range * rescale_ratio

    # smoothing with gaussian filter
    structure_img_smooth_for_seg = image_smoothing_gaussian_slice_by_slice(struct_img_for_seg, sigma=gaussian_smoothing_sigma, truncate_range=gaussian_smoothing_truncate_range)
